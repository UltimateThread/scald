...
namespace SCALD_LOGGER {
   ...
   void Logger::Log(const std::string& message, Args&&... args)
   {
      ...
      if (!m_bInitialized)
      {
         std::cout << "The logger must be initialized before it is used!\n";
         return;
      }

      std::stringstream ss;
      ss << "SCALD [INFO]: " << CurrentDateTime() << " - " << std::vformat(message, std::make_format_args(std::forward<Args>(args)...)) << "\n";
   ...
   void Logger::Warn(const std::string& message, Args&&... args)
   {
      ...
      std::stringstream ss;
      ss << "SCALD [WARN]: " << CurrentDateTime() << " - " << std::vformat(message, std::make_format_args(std::forward<Args>(args)...)) << "\n";
   ...
   void Logger::Error(std::source_location location, const std::string& message, Args&&... args)
   {
      ...
      std::stringstream ss;
      ss << "SCALD [ERROR]: " << CurrentDateTime() << " - " << std::vformat(message, std::make_format_args(std::forward<Args>(args)...)) << 
